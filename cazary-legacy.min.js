...
"source"],FULL:"fontname fontsize;bold italic underline strikethrough removeformat | forecolor backcolor | superscript subscript;justifyleft justifycenter justifyright justifyfull | indent outdent | insertorderedlist insertunorderedlist;inserthorizontalrule insertimage createlink unlink;undo redo;source".split(";")};a(function(a){a(G).bind("click",function(){d()}).bind("keydown",function(a){27===a.keyCode&&d()})});return function(b){b=a.extend({mode:"rte",style:"body{margin:0px;padding:8px;}p{margin:0px;padding:0px;}",
...
N(b);break;default:return null}r&&k.find("li").bind("click",function(){var k=a(this).data("param");g(b,k)});u=J.addClass("cazary-active").offset();u.top+=J.outerHeight();k.addClass("cazary-panel").addClass("cazary-panel-"+b).data("id",K).data("command",b).css({left:u.left+"px",top:u.top+"px"}).bind("click",function(){return!1}).appendTo(a(G.body)).find(":text:first").trigger("focus");return k}function e(b,c){var d=a("<ul />").addClass("cazary-widget-select");a.each(c,function(){var b=this.toString(),
b=a("<li />").attr({unselectable:"on",title:b}).css({"font-family":b}).data("param",b).text(b);d.append(b)});return a("<div>").append(d)}function O(b){var c=a("<ul />").addClass("cazary-widget-select");a.each(v,function(b,k){k=l(k);var d=a("<li />").attr({unselectable:"on",title:k}).data("param",b),e=a("<font />").attr({size:b}).text(k);c.append(d.append(e))});return a("<div>").append(c)}function P(b,c){var d=a("<div>");a.each(c,function(){var b=a("<ul />").addClass("cazary-widget-select-color");
a.each(this,function(){var c=this.toString(),c=a("<li />").attr({unselectable:"on",title:c}).css({"background-color":c}).data("param",c).text(c);b.append(c)});d.append(b)});return d}function M(b){function c(){var a=d.find(".cazary-widget-insertimage-url"),r=a.val();if(!B(r))return a.trigger("focus"),!1;g(b,r);return!1}var d=a("<div>").append(a("<form />").attr("action","#").append(a("<div />").append(a("<fieldset />").append(a("<legend />").text(l("Input image URL"))).append(a('<input type="text" />').addClass("cazary-widget-insertimage-url").attr({required:"required",
placeholder:l("http://example.com/path/to/image.jpg")}))).append(a('<input type="submit" />').addClass("cazary-widget-submit").val(l("Insert")))).append(a("<fieldset />").addClass("cazary-widget-preview").append(a("<legend />").text(l("Preview"))).append(a("<img />").addClass("cazary-widget-preview-insertimage"))));d.find("form").bind("submit",c);d.find(".cazary-widget-submit").bind("click",c);d.find(".cazary-widget-insertimage-url").bind("keydown paste",function(){var b=a(this);t.setTimeout(function(){var a=
b.val(),c=b.data("url_old");a!==c&&(b.data("url_old",a),c=d.find(".cazary-widget-preview"),B(a)?(c.show(),d.find(".cazary-widget-preview-insertimage").attr("src",a)):c.hide())},10)});return d}function N(b){function c(){var a=d.find(".cazary-widget-createlink-url"),e=a.val();if(L(e))e="mailto:"+e;else if(!B(e))return a.trigger("focus"),!1;g(b,e);return!1}var d=a("<div>").append(a("<form />").attr("action","#").append(a("<div />").append(a("<fieldset />").append(a("<legend />").text(l("Input link URL or E-mail address"))).append(a('<input type="text" />').addClass("cazary-widget-createlink-url").attr({required:"required",
placeholder:l("http://example.com/, someone@example.com")}))).append(a('<input type="button" />').addClass("cazary-widget-submit").val(l("Insert")))).append(a("<fieldset />").addClass("cazary-widget-preview").append(a("<legend />").text(l("Preview"))).append(a("<iframe />").addClass("cazary-widget-preview-createlink"))));d.find("form").bind("submit",c);d.find(".cazary-widget-submit").bind("click",c);d.find(".cazary-widget-createlink-url").bind("keydown paste",function(){var b=a(this);t.setTimeout(function(){var a=
b.val(),c=b.data("url_old");a!==c&&(b.data("url_old",a),c=d.find(".cazary-widget-preview"),B(a)?(c.show(),d.find(".cazary-widget-preview-createlink").attr("src",a)):c.hide())},10)});return d}function x(){var a=c.getCurrentStatus(),b;for(b in a){var d=a[b],e=m.find(".cazary-command-"+b);if(b===c.COMMAND_FONTNAME){var f=d;null===f&&(d="",f=e.attr("title"));e.css({"font-family":d}).text(f)}else b===c.COMMAND_FONTSIZE?(f=d,f=null===f?e.attr("title"):l(v[f]),e.text(f)):b===c.COMMAND_FORECOLOR||b===c.COMMAND_BACKCOLOR?
...
y=[c.COMMAND_FONTNAME,c.COMMAND_FONTSIZE,c.COMMAND_FORECOLOR,c.COMMAND_BACKCOLOR,c.COMMAND_INSERTIMAGE,c.COMMAND_CREATELINK];"html"==b.mode?w():h();a(c.contentDocument).bind("select",function(){x()}).bind("mouseup",function(){d();x()}).bind("keydown",function(a){27===a.keyCode&&d()}).bind("keyup paste",function(){t.setTimeout(function(){x();H()},10)});a(c.contentWindow).bind("focus",function(){d();x()}).bind("blur",function(){n.val(c.value())});m.find("ul.cazary-commands-list li").bind("click",function(b){a(this).hasClass("cazary-disabled")&&
(b.stopImmediatePropagation(),F())});z.bind("click",function(){z.hasClass("cazary-active")?h():w();F()});a.each(A,function(){var a=this.toLowerCase();m.find(".cazary-command-"+a).bind("click",function(){g(a)})});a.each(y,function(){var c=this.toLowerCase();m.find(".cazary-command-"+c).bind("click",function(){var d=a(this);C(c,b,d);return!1})})})}}(p)})});
