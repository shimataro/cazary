...
FULL:"fontname fontsize;bold italic underline strikethrough removeformat | forecolor backcolor | superscript subscript;justifyleft justifycenter justifyright justifyfull | indent outdent | insertorderedlist insertunorderedlist;inserthorizontalrule insertimage createlink unlink;undo redo;source".split(";")};a(function(a){a(F).on("click",function(){d()}).on("keydown",function(a){27===a.keyCode&&d()})});return function(b){b=a.extend({mode:"rte",style:"body{margin:0px;padding:8px;}p{margin:0px;padding:0px;}",
...
N(b);break;default:return null}if(w)f.on("click","li",function(){var f=a(this).data("param");h(b,f)});t=I.addClass("cazary-active").offset();t.top+=I.outerHeight();f.addClass("cazary-panel").addClass("cazary-panel-"+b).data("id",J).data("command",b).css({left:t.left+"px",top:t.top+"px"}).on("click",function(){return!1}).appendTo(a(F.body)).find(":text:first").trigger("focus");return f}function e(b,c){var d=a("<ul />").addClass("cazary-widget-select");a.each(c,function(){var f=this.toString(),f=a("<li />").attr({unselectable:"on",
title:f}).css({"font-family":f}).data("param",f).text(f);d.append(f)});return a("<div>").append(d)}function O(b){var c=a("<ul />").addClass("cazary-widget-select");a.each(u,function(b,f){f=l(f);var d=a("<li />").attr({unselectable:"on",title:f}).data("param",b),e=a("<font />").attr({size:b}).text(f);c.append(d.append(e))});return a("<div>").append(c)}function P(b,c){$panel=a("<div>");a.each(c,function(){var b=a("<ul />").addClass("cazary-widget-select-color");a.each(this,function(){var f=this.toString(),
f=a("<li />").attr({unselectable:"on",title:f}).css({"background-color":f}).data("param",f).text(f);b.append(f)});$panel.append(b)});return $panel}function M(b){function c(){var a=$panel.find(".cazary-widget-insertimage-url"),f=a.val();if(!B(f))return a.trigger("focus"),!1;h(b,f);return!1}$panel=a("<div>").append(a("<form />").attr("action","#").append(a("<div />").append(a("<fieldset />").append(a("<legend />").text(l("Input image URL"))).append(a('<input type="text" />').addClass("cazary-widget-insertimage-url").attr({required:"required",
placeholder:l("http://example.com/path/to/image.jpg")}))).append(a('<input type="submit" />').addClass("cazary-widget-submit").val(l("Insert")))).append(a("<fieldset />").addClass("cazary-widget-preview").append(a("<legend />").text(l("Preview"))).append(a("<img />").addClass("cazary-widget-preview-insertimage"))));return $panel.on("submit","form",c).on("click",".cazary-widget-submit",c).on("keydown paste",".cazary-widget-insertimage-url",function(){var b=a(this);p.setTimeout(function(){var a=b.val(),
c=b.data("url_old");a!==c&&(b.data("url_old",a),c=$panel.find(".cazary-widget-preview"),B(a)?(c.show(),$panel.find(".cazary-widget-preview-insertimage").attr("src",a)):c.hide())},10)})}function N(b){function c(){var a=d.find(".cazary-widget-createlink-url"),e=a.val();if(K(e))e="mailto:"+e;else if(!B(e))return a.trigger("focus"),!1;h(b,e);return!1}var d=a("<div>").append(a("<form />").attr("action","#").append(a("<div />").append(a("<fieldset />").append(a("<legend />").text(l("Input link URL or E-mail address"))).append(a('<input type="text" />').addClass("cazary-widget-createlink-url").attr({required:"required",
...
c.COMMAND_INSERTIMAGE,c.COMMAND_CREATELINK];"html"==b.mode?q():k();a(c.contentDocument).on("select",function(){v()}).on("mouseup",function(){d();v()}).on("keydown",function(a){27===a.keyCode&&d()}).on("keyup paste",function(){p.setTimeout(function(){v();G()},10)});a(c.contentWindow).on("focus",function(){d();v()}).on("blur",function(){n.val(c.value())});m.on("click","ul.cazary-commands-list li",function(b){a(this).hasClass("cazary-disabled")&&(b.stopImmediatePropagation(),E())});z.on("click",function(){z.hasClass("cazary-active")?
k():q();E()});a.each(A,function(){var a=this.toLowerCase();m.on("click",".cazary-command-"+a,function(){h(a)})});a.each(x,function(){var c=this.toLowerCase();m.on("click",".cazary-command-"+c,function(){var d=a(this);L(c,b,d);return!1})})})}}(q)})})(jQuery,window);